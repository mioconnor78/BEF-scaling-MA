...
modBtrophic3<-lmer(logY.rs ~ logSc*log(Tscale) + logSc*Sys1*factor(TG1) - logSc:Sys1T:factor(TG1)2 +
                     (1 + logSc|Entry) + (1 + logSc|ExptA) + (1 + logSc|Study), data=data, REML = FALSE, na.action=na.omit)
...
modBtrophicPred.varyNone$logY.rs.predicted <-  fixef(modBtrophic)[1] + fixef(modBtrophic)[2]*modBtrophicPred.varyNone$logSc + mean(SST5$Tscale)*fixef(modBtrophic)[3]
...
###The master plot
fitted_plot <- ggplot() +
  geom_line(data=SST5, aes(x=exp(logSc), y=exp(modBtrophic.fitted), group=Entry), color="lightgrey") +
...
fitted_plot3+
  geom_line(data=modBtrophic3Pred.varyall, mapping=aes(x=exp(logSc), y=exp(logY.rs.predicted)))
