...
SST <- metamaster3 %>% filter(Ygen == 'SST') # down to 1 T H study
SST <- SST %>% filter(value.st != 'NaN')
...
SST <- SST %>% filter(TDBU == 'TD') # removing 'bottom up' studies; the last T herb study goes out at this stage.
...
SST2 <- SST1 %>% filter(Slevels > 1)
SST2 <- SST2 %>% filter(HigherT != "")
SST2 <- SST2 %>% filter(HigherT != ".")
# get rid of Tscale vals = 0
SST2 <- SST2 %>% filter(Tscale!="")
SST2 <- SST2 %>% filter(Yunits!='proportional change')
...
dim(SST4)
SST4 <- subset(SST4, SST4$Study!=83, select=1:(n+13), drop=TRUE)
SST4 <- subset(SST4, SST4$Study!=177, select=1:(n+13), drop=TRUE) 
...
#remove carnivores
SST5 <- subset(SST4, SST4$TG1!="3", select=1:(n+18), drop=TRUE) 

## clean it up for posting
SST5 <- SST5[,-c(5,37:39,41, 43)]
