...
### April 2015: using this for final analysis, includes 3 random effects levels and standardized coefs
#######################################################################
...
modBasici <- lmer(logY.rs ~ logSc*log(Tscale) +  (1|ExptA)  + (1|Study), data=data, REML = FALSE, na.action=na.omit)
modBasicii <- lm(logY.rs ~ logSc*log(Tscale), data=data, na.action=na.omit)

# basic2 (The level-1 model w/ time and logSc)
modBasic2 <- lmer(logY.rs ~ logSc+log(Tscale) + (1 + logSc|Entry) + (1 + logSc|ExptA) + (1 + logSc|Entry) + (1 + logSc|Study), data=data, REML = FALSE, na.action=na.omit)
...
# Full model 
modFM<-lmer(logY.rs ~ logSc*log(Tscale) + logSc*Sys1  + logSc*TG1 + logSc*unit.types2 + logSc*HigherT + logSc*log(Smax) + logSc*restrt + logSc*log(MaxTscale+1) + (1 + logSc|Entry) + (1 + logSc|ExptA)  + (1 + logSc|Study), data=data, REML = FALSE, na.action=na.omit)
